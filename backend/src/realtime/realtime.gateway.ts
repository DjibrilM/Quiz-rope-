  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  handleJoinMatch(
    ...
    @MessageBody() data: { matchId: string; playerId: string; teamSide: string },
  ) {
  ...
  handleSessionWatch(
    ...
    client.join(room);
    this.logger.log(`Client ${client.id} watching session ${data.sessionToken}`);
  }

  emitSessionAuthorized(sessionToken: string, data: { parentId: string; childId?: string | null }) {
