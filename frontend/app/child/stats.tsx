...
function SubjectRow({
  ...
  const accuracyColor =
    stat.accuracy >= 70 ? "#10B981" : stat.accuracy >= 40 ? "#F59E0B" : "#EF4444";
...
export default function ChildStatsScreen() {
  ...
  const { data, isLoading, isError, error, refetch: refetchStats } = useQuery({
  ...
  const { data: matchHistory = [], refetch: refetchHistory } = useQuery({
  ...
  const soloCount = matchHistory.filter((m) => m.gameMode === "solo").length;
  const splitscreenCount = matchHistory.filter((m) => m.gameMode === "splitscreen").length;
  const soloAccuracy =
    soloCount > 0
      ? Math.round(
          matchHistory
            .filter((m) => m.gameMode === "solo")
            .reduce((sum, m) => sum + m.accuracy, 0) / soloCount,
        )
      : null;
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl
            ...
            onRefresh={onRefresh}
            tintColor={FORTNITE_COLORS.glowPurple}
          />
        }
      >
        {isLoading && (
        ...
        {!isLoading && !isError && data && data.totalMatches > 0 && (
          <View style={{ gap: 16, paddingTop: 8 }}>
            ...
            {/* Overall stats — 2×2 grid for richer info */}
            {(() => {
              const totalCorrect = data.subjectStats.reduce(
                ...
                <View style={{ gap: 10 }}>
                  <View style={{ flexDirection: "row", gap: 10 }}>
                    <StatCard
                      ...
                      value={String(matchHistory.length || data.totalMatches)}
                    />
                    <StatCard
                ...
                <View style={{ gap: 6 }}>
                  <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                      ...
                      >
                        <Text style={{ fontSize: 11, fontFamily: FONTS.bodySemiBold, color: "#A78BFA" }}>
                          SOLO
                        </Text>
                      </View>
                      <Text style={{ fontSize: 14, fontFamily: FONTS.bodyBold, color: FORTNITE_COLORS.textPrimary }}>
                        {soloCount} {soloCount === 1 ? "session" : "sessions"}
                      </Text>
                    </View>
                    {soloAccuracy !== null && (
                      <Text
                        style={{
                          fontSize: 14,
                          fontFamily: FONTS.accent,
                          color: soloAccuracy >= 70 ? "#10B981" : soloAccuracy >= 40 ? "#F59E0B" : "#EF4444",
                        }}
                      >
                        {soloAccuracy}% avg
                      </Text>
                  ...
                  {soloCount > 0 && soloAccuracy !== null && (
                    <View style={{ height: 5, backgroundColor: "#3D2E4A", borderRadius: 3, overflow: "hidden" }}>
                      <View
                        style={{
                          height: "100%",
                          width: `${soloAccuracy}%` as any,
                          backgroundColor: soloAccuracy >= 70 ? "#10B981" : soloAccuracy >= 40 ? "#F59E0B" : "#EF4444",
                          borderRadius: 3,
                        }}
                      />
                    </View>
                ...
                {splitscreenCount > 0 && (
                  <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                      ...
                      >
                        <Text style={{ fontSize: 11, fontFamily: FONTS.bodySemiBold, color: "#F59E0B" }}>
                          SPLIT
                        </Text>
                      </View>
                      <Text style={{ fontSize: 14, fontFamily: FONTS.bodyBold, color: FORTNITE_COLORS.textPrimary }}>
                        {splitscreenCount} {splitscreenCount === 1 ? "session" : "sessions"}
                      </Text>
                    </View>
                  </View>
                )}
